...
* Type 1 error: We reject H₀ (say continents are different), but in reality, they are not. Which means we wrongly claim that some continents have different death rates, and maybe governments overreact or misallocate disaster resources.
...
The data is very skewed. The dot plots below show the variance in the data. The number of deaths per earthquake has a high level of randomness, as shown in the plots. We will create the plots for Americas first.
```{r}
...
Then we wanna get the dot plot for Asia and see the difference between them
```{r}
...
After we check the dot plot, we notice high levels of variance, these data are independent, but we are not sure if data is normal. We test if data is normal below:

To prove that the data is not normalized, we use the shapiro test. The p-values below are less than 0.05, which suggests both sets of data are not normal.
```{r}
shapiro.test(data_Americas$deaths)
shapiro.test(data_Asia$deaths)
```

We run data through t.test. While the difference in mean looks big enough to reject null hypothesis, the data is very noisy (not normalized), making it hard to prove anything. As seen below, the confidence intervals are extremely wide, and we cant prove anything statistically. The difference in average death might just be random.
```{r}
# t.test
