...
Based on the graph, we would assume that the death tolls greatly differ between continents. But does this data meet the necessary conditions?

The data is independent, but after running Shapiro-Wilk test, we find that the data is not normal.
...
In the context of our research question, we will compare the average death tolls of the top 2 continents. In this case, Americas and Asia have the highest average death tolls.



```{r}
# Bar chart of average deaths
ggplot(region_summary, aes(x = reorder(continent, -avg_deaths), y = avg_deaths)) +
...
After we check the graph, we run data through t.test. While the difference in mean looks big enough to reject null hypothesis, the data is very noisy and not normal, making it hard to prove anything. As seen below, the confidence intervals are extremely wide, and we cant prove anything statistically. The difference in averageg death might just be random.
```{r}
...
## Conclusion:
Since our p-value = 0.5544 > 0.05, so we fail to reject the null hypothesis, which means: There is not enough evidence to reject the null hypothesis, so average number of deaths per continent does not differ.
