...
The data is very right skewed. The dot plots below show the variance in the data. The number of deaths per earthquake has a high level of randomness, as shown in the plots. We will create the plots for Americas first.
```{r}
...
This is the dot plot for Asia, which is also very right skewed.
```{r}
...
After we check the dot plot, we notice high levels of variance. The data is independent, but we are not sure if data is normal. We test if data is normal below:
